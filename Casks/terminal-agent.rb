# This file was generated by GoReleaser. DO NOT EDIT.
cask "terminal-agent" do
  version "0.46.2"

  on_macos do
    on_intel do
      sha256 "009b0ed318f2d5f2dafdf33fd68648cfeb5d78c4b3cadff9ecb7b9ca0d9f0532"
      url "https://github.com/laszukdawid/terminal-agent/releases/download/v#{version}/terminal-agent_Darwin_x86_64.tar.gz"
    end
    on_arm do
      sha256 "c0cf9e5e724c075addc82b9378bb111ce825e6465241945284fdb9824e0cf537"
      url "https://github.com/laszukdawid/terminal-agent/releases/download/v#{version}/terminal-agent_Darwin_arm64.tar.gz"
    end
  end

  on_linux do
    on_intel do
      sha256 "3400365cb702edeace4eb08a399804a239d9e754ab7e891464196eb722b8c49a"
      url "https://github.com/laszukdawid/terminal-agent/releases/download/v#{version}/terminal-agent_Linux_x86_64.tar.gz"
    end
    on_arm do
      sha256 "0e25f71206c2a2175a18762c68ed0a7b29ad6fa4e12e7c75d43b7b525c83046d"
      url "https://github.com/laszukdawid/terminal-agent/releases/download/v#{version}/terminal-agent_Linux_arm64.tar.gz"
    end
  end

  name "terminal-agent"
  desc "An LLM Agent to help you from and within the terminal"
  homepage "https://github.com/laszukdawid/terminal-agent"

  livecheck do
    skip "Auto-generated on release."
  end

  binary "agent"

  # No zap stanza required

  caveats <<~EOS
    To get started with Terminal Agent:

    1. Set up your API key for your preferred provider:
       export OPENAI_API_KEY=your-key-here
       # or
       export ANTHROPIC_API_KEY=your-key-here

    2. Configure your provider:
       agent config set provider openai
       agent config set model gpt-4o-mini

    3. Start asking questions:
       agent ask "What is a file descriptor?"

    For more information, visit:
      https://laszukdawid.github.io/terminal-agent/

    Pro tip: Set up an alias for quick access:
      alias aa="agent ask"
  EOS
end
